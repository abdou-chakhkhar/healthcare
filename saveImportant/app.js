...
function verifyAssetData(org, resultBuffer, expectedId, color, size, ownerUserId, appraisedValue) {
    ...
    let asset;
    console.log("apppppppppppppppppppppppppppppp", resultBuffer.toString());
    if (resultBuffer) {
...
async function main() {
    try {
        ...
        try {
            ...
            let asset3Data = { objectType: assetType, assetID: assetID3, color: 'yellow', size: 18, appraisedValue: 200 };
            let asset4Data = { objectType: assetType, assetID: assetID4, color: 'red', size: 50, appraisedValue: 500 };
            ...
            console.log('\n**************** As Org2 Client ****************');
            let statefulTxn2 = contractOrg2.createTransaction('CreateAsset');
            //Add asset3
            console.log('\n--> Submit Transaction: CreateAsset ' + assetID3);
            statefulTxn2 = contractOrg2.createTransaction('CreateAsset');
            tmapData = Buffer.from(JSON.stringify(asset3Data));
            statefulTxn2.setTransient({
                asset_properties: tmapData
            });
            result = await statefulTxn2.submit();

            //Add asset3
            console.log('\n--> Submit Transaction: CreateAsset ' + assetID4);
            statefulTxn2 = contractOrg2.createTransaction('CreateAsset');
            tmapData = Buffer.from(JSON.stringify(asset4Data));
            statefulTxn2.setTransient({
                asset_properties: tmapData
            });
            result = await statefulTxn2.submit();
            ...
            console.log('\n~~~~~~~~~~~~~~~~ As Org1 Client (read his asset)~~~~~~~~~~~~~~~~');
            console.log('\n--> Evaluate Transaction: ReadAsset ' + assetID1);
            result = await contractOrg1.evaluateTransaction('ReadAsset', assetID1);
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            verifyAssetData(mspOrg1, result, assetID1, 'green', 20, Org1UserId);

            console.log('\n~~~~~~~~~~~~~~~~ As Org1 Client (read org2 asset)~~~~~~~~~~~~~~~~');
            console.log('\n--> Evaluate Transaction: ReadAsset ' + assetID1);
            result = await contractOrg1.evaluateTransaction('ReadAsset', assetID3);
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            verifyAssetData(mspOrg1, result, assetID3, 'yellow', 18, Org2UserId);

            console.log('\n~~~~~~~~~~~~~~~~ As Org2 Client (read his asset)~~~~~~~~~~~~~~~~');
            console.log('\n--> Evaluate Transaction: ReadAsset ' + assetID3);
            result = await contractOrg2.evaluateTransaction('ReadAsset', assetID3);
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            verifyAssetData(mspOrg2, result, assetID3, 'yellow', 18, Org2UserId);


            // Org1 can ReadAssetPrivateDetails from Org1's private collection
            console.log('\n~~~~~~~~~~~~~~~~ As Org1 Client (read org1 privateasset)~~~~~~~~~~~~~~~~');
            let result1 = await contractOrg1.evaluateTransaction('ReadAssetPrivateDetails', org1PrivateCollectionName, assetID1);
            console.log("--------------------------Org1 read its private data -------------", result1.toString());

            // // Org1 cannot ReadAssetPrivateDetails from Org2's private collection
            // console.log('\n~~~~~~~~~~~~~~~~ As Org1 Client (cannot read org2 s privateasset)~~~~~~~~~~~~~~~~');
            // let result2 = await contractOrg1.evaluateTransaction('ReadAssetPrivateDetails', org2PrivateCollectionName, assetID3);
            // console.log("--------------------------Org1 read org2 private data -------------", result2.toString());
