...
async function readPrivateAsset(assetKey, org, contract) {
	...
	try {
		...
		const asset = JSON.parse(resultBuffer.toString());
...
function checkAsset(org, resultBuffer, ownerOrg) {
	...
	if (asset) {
		if (asset.OwnerOrg === ownerOrg) {
			console.log(`*** Result from ${org} - asset ${asset.ID} owned by ${asset.OwnerOrg} DESC:${asset.publicDescription}`);
		} else {
			console.log(`${RED}*** Failed owner check from ${org} - asset ${asset.ID} owned by ${asset.OwnerOrg} DESC:${asset.publicDescription}${RESET}`);
		}
	}
}
...
async function main() {
	...
	try {
		...
		try {
			...
			console.log(`read the public details by both orgs`);
			// read the public details by both orgs
			await readAssetByBothOrgs(assetKey, org1, contractOrg1, contractOrg2);
