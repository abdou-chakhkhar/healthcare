...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "Creating insProvider Identities"
  ...
  infoln "Generating CCP files for the all Orgs"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.hospital docker_peer0.pharmacy docker_peer0.insProvider docker_peer0.laboratory
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/insProvider/msp organizations/fabric-ca/insProvider/tls-cert.pem organizations/fabric-ca/insProvider/ca-cert.pem organizations/fabric-ca/insProvider/IssuerPublicKey organizations/fabric-ca/insProvider/IssuerRevocationPublicKey organizations/fabric-ca/insProvider/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/laboratory/msp organizations/fabric-ca/laboratory/tls-cert.pem organizations/fabric-ca/laboratory/ca-cert.pem organizations/fabric-ca/laboratory/IssuerPublicKey organizations/fabric-ca/laboratory/IssuerRevocationPublicKey organizations/fabric-ca/laboratory/fabric-ca-server.db'
